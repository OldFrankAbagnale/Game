...
namespace Maxx53.Games
{
    class SnakeGame
    {
        ...
        private class PredatorSegment : GameObject
        {
            public PredatorSegment(int x, int y, int imgN, Direction dir) : base(x, y, imgN)
            {
                //Добавляем всего одной свойство - направление, в которое повернута текстура
                Dir = dir;
            }
            ...
            //Метод для удобного копирования сегмента
            public PredatorSegment Copy()
            {
                return new PredatorSegment(Pos.X, Pos.Y, imgIndex, Dir);
            }
        ...
        class GameLevel
        {
            ...
            //Всего 4 свойства. Стартовая позиция змеи, ее направление, список координат камней, направление хищника
            public Direction PredatorDir { set; get; }
            public Direction Dir { set; get; }
        ...
        private static Image predator_texture = new Bitmap(texturePath + "snake_texture.png");
        ...
        private static List<Image> predatorTexList = new List<Image>();
        ...
        private List<PredatorSegment> predator = new List<PredatorSegment>();
        ...
        private Direction predatorDirection = Direction.Up;
        ...
        private void LoadPredatorTex()
        {
            predatorTexList.Clear();

            var sourceWidth = predator_texture.Width;
            ...
            for (int i = 3; i >= 0; i--)
            {
                ...
                //Первая картника, которая уже смотрит вниз, ее вращать не требуется
                var source = Utils.ResizeImage(Utils.CropImage(predator_texture, cropArea), PixelLen, PixelLen);
                ...
                //Добавляем в список 4 копии текстуры, каждая повернута в своем направлении
                predatorTexList.Add(source);
                predatorTexList.Add(source270);
                predatorTexList.Add(source90);
                predatorTexList.Add(source180);
            }
            ...
            //Добавляем 4 копии головы (по 1й для каждого направления) для создания эффекта смерти
            predatorTexList.Add(new Bitmap(predatorTexList[0], new Size(PixelLen, halfPix)));
            predatorTexList.Add(new Bitmap(predatorTexList[1], new Size(halfPix, PixelLen)));
            predatorTexList.Add(new Bitmap(predatorTexList[2], new Size(halfPix, PixelLen)));
            predatorTexList.Add(new Bitmap(predatorTexList[3], new Size(PixelLen, halfPix)));
        }
        ...
        private void SpawnPredator(int x, int y, Direction dir)
        {
            //Чистим список сегментов
            predator.Clear();
            ...
            switch (dir)
            {
                case Direction.Up:
                    //Добавляем голову, смотрящую вверх
                    predator.Add(new PredatorSegment(x, y, 3, dir));
                    //Добавляем хвост, смотрящий назад
                    predator.Add(new PredatorSegment(x, y + 1, 11, dir));
                    break;

                //Остальные направления по аналогии
                case Direction.Down:
                    predator.Add(new PredatorSegment(x, y, 0, dir));
                    predator.Add(new PredatorSegment(x, y - 1, 8, dir));
                    break;
                case Direction.Left:
                    predator.Add(new PredatorSegment(x, y, 2, dir));
                    predator.Add(new PredatorSegment(x + 1, y, 10, dir));
                    break;
                case Direction.Right:
                    predator.Add(new PredatorSegment(x, y, 1, dir));
                    predator.Add(new PredatorSegment(x - 1, y, 9, dir));
                    break;
            }
        }
        ...
        private void MovePredator() {
            //Вычисляем напраление
            PredatorStep();
            //Запоминаем позицию и направление головы до следующего шага
            //Понадобится для определения определения направления следующего сегмента
            Point oldHeadPos = predator[0].Pos;
            Direction oldDir = predator[0].Dir;

            //Теперь меняем позицию головы на один пиксель в сторону, в зависимости от текущего направления
            switch (predatorDirection)
            {
                case Direction.Right:
                    predator[0].Pos = new Point(predator[0].Pos.X + 1, predator[0].Pos.Y);
                    break;
                case Direction.Left:
                    predator[0].Pos = new Point(predator[0].Pos.X - 1, predator[0].Pos.Y);
                    break;
                case Direction.Up:
                    predator[0].Pos = new Point(predator[0].Pos.X, predator[0].Pos.Y - 1);
                    break;
                case Direction.Down:
                    predator[0].Pos = new Point(predator[0].Pos.X, predator[0].Pos.Y + 1);
                    break;
            ...
            //Задаем направление и изображение с учетом направления
            predator[0].Dir = predatorDirection;
            //В методе ChangeImg, 0 - указывает на текстуру головы
            predator[0].ChangeImg(0);

            //В обратном цикле (начинаем с последнего элемента списка) прогоняем все точки змейки, кроме головы
            for (int i = predator.Count - 1; i > 0; i--)
            {
                //Сдвигаем каждую точку тела на позицию предыдущей точки в списке. Получается эффект движения.
                //В итоге, вторая точка займет место первой точки (головы), голову двигаем отдельно
                ...
                {
                    //Двигаем сегмент на позицию предыдущего
                    predator[i] = predator[i - 1].Copy();

                    //Если попался хвост
                    if (i == predator.Count - 1)
                    {
                        //Определеяем направление хвоста (костыль)
                        if (predator.Count > 3)
                            predator[i].Dir = predator[predator.Count - 3].Dir;
                        else
                            predator[i].Dir = oldDir;

                        //Меняем изображение сегмента на хвост с учетом направления
                        predator[i].ChangeImg(8);
                    }
                ...
                else
                {
                    ...
                    predator[1].Pos = oldHeadPos;
                    predator[1].Dir = oldDir;

                    //Если хвост
                    if (i == predator.Count - 1)
                    {
                        //Задаем ему направление головы
                        predator[1].Dir = predator[0].Dir;
                        //Меняем изображение на хвост, с учетом направления
                        //8 - указывает на позицию текстуры с хвостом в списке
                        predator[1].ChangeImg(8);
                    }
                    else
                    {
                        //Если другой сегмент, прямое тело или поворот

                        //Если направление головы не равно направлению первого после головы сегмента, значит хищник поворачивает!
                        //Определеям направление поворота, чтобы задать сегменту правильное изображение
                        if (predator[0].Dir != predator[1].Dir)
                        {
                            if (((oldDir == Direction.Left) && (predator[0].Dir == Direction.Down)) || ((oldDir == Direction.Up) && (predator[0].Dir == Direction.Right)))
                            {
                                //90 градусов
                                predator[1].imgIndex = 14;
                            }
                            else
                                if (((oldDir == Direction.Right) && (predator[0].Dir == Direction.Down)) || ((oldDir == Direction.Up) && (predator[0].Dir == Direction.Left)))
                            {
                                //180 градусов
                                predator[1].imgIndex = 15;
                            }
                            else
                                    if (((oldDir == Direction.Right) && (predator[0].Dir == Direction.Up)) || ((oldDir == Direction.Down) && (predator[0].Dir == Direction.Left)))
                            {
                                //270 градусов
                                predator[1].imgIndex = 13;
                            }
                            else
                                //0 градусов
                                predator[1].imgIndex = 12;

                        }
                        else
                            //Если хищник не поворачивает, рисуем текстуру прямого тела с учетом направления
                            predator[1].ChangeImg(4);
                    }
                }
            }

            //Проверяем наличие столкновений головы хищника с игровыми объектами
            //CheckCollisions();
        }
        private void PredatorStep() {
            //изменение направления не каждый тик
            ...
            if (predatorStep % 3 == 0)
            {
                ...
                if (predatorStep % 2 == 0)
                {
                    if (snake[0].Pos.X > predator[0].Pos.X)
                    {
                        predatorDirection = Direction.Right;
                    }
                    else { predatorDirection = Direction.Left; }
                }
            }
            else
            {

                if (snake[0].Pos.Y > predator[0].Pos.Y)
                {
                    predatorDirection = Direction.Down;
                }
                else { predatorDirection = Direction.Up; }
            }
            predatorStep++;
        ...
        private void CheckCollisions()
        {
            ...
            //Проверяем наличие столкновений головы змейки со хищником

            for (int j = 1; j < snake.Count; j++)
            {
                if (predator[0].Pos == snake[j].Pos)
                {
                    //Воткнулись в себя, умерли
                    Die();
                }
        ...
        private void canvas_Paint(object sender, PaintEventArgs e)
        {
            ...
            //Рисуем текстуры сегментов хищника
            for (int i = predator.Count - 1; i >= 0; i--)
            {
                //Изображение хватаем из списка, по номеру который хранится в поле класса imgIndex
                g.DrawImage(predatorTexList[predator[i].imgIndex], predator[i].imgPos);
            }
